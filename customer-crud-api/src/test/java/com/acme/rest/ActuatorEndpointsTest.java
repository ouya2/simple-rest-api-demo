...
public class ActuatorEndpointsTest extends EndpointTest {
  ...
  public void getInfo() throws Exception {
    mockMvc.perform(get("/info"))
        ...
        .andExpect(status().isOk())
        .andExpect(content().contentType(JSON_MEDIA_TYPE))
        .andExpect(jsonPath("$.build.artifact").value("customer-crud-api"))
        .andExpect(jsonPath("$.build.name").value("Customer Crud API"))
        .andExpect(jsonPath("$.build.description").value("Customer Service"));
  }
